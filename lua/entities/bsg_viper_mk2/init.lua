...
function ENT:SpawnFunction( ply,tr )
	...
	local ent = ents.Create("bsg_viper_mk2") --SpaceShip entity
...
function ENT:Initialize()
	...
	self.Entity:SetModel("models/katharsmodels/vipermk2/vipermk2.mdl")
	...
	if self.Piloting and IsValid(self.Pilot) then
		...
		local speedvalue=0
		 ...
		 local move = { }
			...
			local ang = self.Pilot:GetAimVector():Angle() + Angle(0,180,0) + Angle((self.Pilot:GetAimVector():Angle().p)*-2,0,0 ) -- first add to fix model and second add to fix up/down problems
			move.angle			= ang
