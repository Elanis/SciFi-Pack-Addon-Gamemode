...
function ENT:SpawnFunction( ply,tr )
	...
	local ent = ents.Create("b5_victory") --SpaceShip entity
	ent:SetPos( tr.HitPos + Vector(0,0,750))
	ent:Spawn()
	ent:Activate()
	return ent
...
function ENT:Initialize()
	...
	self.Entity:SetModel("models/mymodel/victory/victory.mdl")
	...
	if self.Piloting and IsValid(self.Pilot) then
		...
		local speedvalue=0
		 ...
		 local move = { }
			...
			local ang = self.Pilot:GetAimVector():Angle() + Angle(0,180,0) -  Angle( self.Pilot:GetAimVector():Angle() .p*2 , 0 , 0 )
			move.angle			= ang
